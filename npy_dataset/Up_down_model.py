...
parent_folder = "./Left_Right_model"
...
for sub_folder in sub_folders:
    ...
    for npy_file in os.listdir(sub_folder_path):
        if npy_file.endswith(".npy"):
            ...
            data.append(np.load(npy_file_path,allow_pickle=True).transpose())
...
model = Sequential()
model.add(Conv1D(256, (4), input_shape=train_X.shape[1:]))
model.add(Activation('relu'))
...
if score[1] >= 0.50:
  model.save("Three-direction-LRN-model-acc-%.2f" % (score[1]*100)+".h5")
  pickle.dump(model, open("Three-direction-LRN-model-acc-%.2f" % (score[1]*100)+".pkl", 'wb'))
